...
/*CRC检验的基本思想是利用线性编码理论，
在发送端根据要传送的k位二进制码序列，
以一定的规则产生一个检验码r位(就是CRC码)，
附在信息后面，构成一个新的二进制码序列数共(k+r)位，
最后发送出去。接收端根据同样的规则校验，以确定传送中是否出错。*/
...
/* crc-32 parity ---------------------------------------------------------------
* compute crc-32 parity for novatel raw(novatel原始观测值校验码计算)
...
/* crc-24q parity --------------------------------------------------------------
* compute crc-24q parity for sbas, rtcm3(sbas, rtcm3校验码计算)
* args   : unsigned char *buff I data
*          int    len    I      data length (bytes)
...
/* crc-16 parity ---------------------------------------------------------------
* compute crc-16 parity for binex, nvs(binex, nvs校验码计算)
* args   : unsigned char *buff I data
*          int    len    I      data length (bytes)
...
* check parity and decode navigation data word(类似观测值校验)
